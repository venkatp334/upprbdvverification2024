...
$(document).ready(function() {
    ...
    $('#theme-selector div.theme').on('click', function(evt) {
        ...
        $('<link>').attr({
            rel:  'stylesheet',
            href: 'css/theme/default/' + theme + '.css?k=' + key
        }).on('load', function() {
        ...
        geocoder.geocode({address:city_name}, function(result, status) {
            if (status === 'OK') {
                var marker = new google.maps.Marker({
                    map:       map,
                    position:  new google.maps.LatLng(result[0].geometry.location.jb, result[0].geometry.location.kb),
...
var buildContribution = function(container, total) {
    ...
    for (var i = 0; i <= total; i++) {
        ...
        $('<div>').addClass('level level' + random_level).attr('title', random_commit + ' commits').appendTo(container);
    }
};
...
var dashboardPage = function() {
    // line chart
    $.plot($("#dashboard-line-chart"), project_chart.data, project_chart.options);
